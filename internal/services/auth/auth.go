...
func (a *Auth) Login(ctx context.Context, email string, password string, appID int) (string, error) {
	...
	if err != nil {
		if errors.Is(err, storage.ErrUserNotFound) {
			a.log.Info("user not found", sl.Err(err))
	...
	if err != nil {
		if errors.Is(err, storage.ErrAppNotFound) {
			a.log.Info("app not found", sl.Err(err))
	...
	log.Info("user logged in successfully", slog.Int64("user_id", user.ID), slog.Int64("app_id", app.ID))
}
